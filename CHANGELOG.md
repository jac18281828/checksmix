...
* Mnemonic parsing no longer depends on accidental backtracking. All 264 `mnemonic_*` grammar rules gained an `!ASCII_ALPHANUMERIC` word-boundary guard and atomic marking, so `GET` can no longer shadow `GETA` — 115 shadowing mnemonics across 199 ordered prefix pairs, of which 193 now have both members assembled from source by a test or the corpus. A baseline swap confirmed no currently-correct routing changed. The six unexercised pairs are the ones involving `SYNCD`/`SYNCDI`/`SYNCID`/`SYNCIDI`, which are never assembled from source — each handler advances the PC and returns, changing no observable state, as `examples/all_instructions_test.mms` itself records. Known residual: the guard does not cover `_`, so a label like `Halt_Loop` still misparses
...
* `examples/all_instructions_test.mms` grew by 940 lines to close corpus coverage gaps, and four vacuous tests within it were fixed — Tests 200 (`NXORI`) and 214 (`MXOR`), which never cleared `Result` before asserting on it, and Tests 259/260 (`PUSHJ`/`PUSHGO`), which could not fail if `POP` never returned
