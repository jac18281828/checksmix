...
* `checksmix run` on a `.mmo` no longer prints a `Debug: instr@0x370` line ahead of the initial state dump. The load path carried a hardcoded read of address `0x370`, left from tracing a `big_fib` failure, and it fired on every object file whatever the program
