...
* **Breaking: `MMix` and `Debugger` are no longer `Send` or `Sync`.** `MMix` now owns its I/O as a `Box<dyn Host>`, and `Debugger` holds an `MMix`. This is deliberate rather than incidental — the intended embedders are single-threaded and capture into `Rc<RefCell<_>>`, which a `Send` bound would forbid — but it is why this release is `0.3.0` and not `0.2.24`. Construct an `MMix` on the thread that runs it
...
* Mnemonic parsing no longer depends on accidental backtracking. All 264 `mnemonic_*` grammar rules gained an `!ASCII_ALPHANUMERIC` word-boundary guard and atomic marking, so `GET` can no longer shadow `GETA` (and 114 other collision groups, 199 pairs in total, now covered by regression tests). A baseline swap confirmed no currently-correct routing changed. Known residual: the guard does not cover `_`, so a label like `Halt_Loop` still misparses
* `STCO`/`STCOI` fixed. `parse_inst_stco_rrr` and `_rri` discarded two `parts.next()` calls on the assumption they were comma tokens, but `comma` is a silent grammar rule that emits no pair — so operand fields shifted. The `_rrr` form ran off the end and panicked during assembly; `_rri` silently landed on a real-but-wrong field
...
* `examples/all_instructions_test.mms` grew by 939 lines to close corpus coverage gaps, and three vacuous tests within it were fixed
* New test for integer `CMP`/`CMPU` (0x30/0x32), which had no dedicated coverage — only the floating-point `FCMP`/`FCMPE` did. `CMPI`/`CMPUI` remain uncovered
